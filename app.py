...
if lang == "العربية":
    ...
    st.markdown("*(Opuntia ficus-indica)* نظام هندسي مخصص لتحديد الجرعات الميدانية للبيوهيدروجين المستخلص من ألواب الصبار لحماية التربة ومحاربة الإجهاد المائي، مع حساب الجدوى الاقتصادية الشاملة.")
    ...
    # غراسة التين الشوكي عادة تتطلب شتلة لكل 4 إلى 5 متر مربع حسب الكثافة
    plants_needed = int(area / 4) 
    water_needed_m3 = area * 0.12 # الصبار مقاوم للجفاف ويحتاج رياً قليلاً نسبياً
    ...
    with col1:
        ...
        st.write("تثمين صبار التين الشوكي واستغلال مادة الصمغ والموسيلاج (Mucilage) الطبيعي لاستخلاص جطبة هيدروجينية حيوية ذات قدرة فائقة على الامتصاص.")
    ...
    with tab1:
        ...
        st.success(***التكلفة الإجمالية للمشروع:** {total_project_cost:,.0f} دج*)
    ...
    with tab2:
        ...
        if net_profit > 0:
            st.success(***صافي الأرباح (الفائدة):** {net_profit:,.0f} دج - المشروع ذو جدوى اقتصادية وفلاحية عالية جداً وموجه لتحقيق الأمن الغذائي!*)
        else:
            st.warning(***ملاحظة اقتصادية:** التكاليف تتجاوز المداخيل المتوقعة، يرجى إعادة ضبط أسعار التسويق أو مساحة الإنتاج.*)
